...
public class TrainAnimalClassifier {
		...
		try {
			ImageClassifier<BufferedImage> classifier =
					NeuralNetImageClassifier.builder()
							...
							.networkArchitecture(Paths.get("src/main/resources/animal.json"))// architecture of the convolutional neural network in json
							.maxError(0.03f) // error level to stop the training (maximum acceptable error)
							.maxEpochs(121) // maximum number of training iterations (epochs)
